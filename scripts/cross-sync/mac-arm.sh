...
-    "custom_vars": {},
+    "custom_vars": {"host_os":"mac", "checkout_mac": True, "checkout_linux": False, "target_os": "mac", "target_cpu": "arm64", "checkout_arm": True, "checkout_arm64": True},
   },
 ]
+target_os = ['mac']
EOF
...
(cd "$BASE_PATH"; tar --zstd -cf mac-arm.tzstd --anchored --exclude-from="$SCRIPT_DIR/tar_excludes_os.txt" --exclude-from="$SCRIPT_DIR/tar_thirdparty_includes.txt" --files-from="$SCRIPT_DIR/tar_includes_os.txt")
