...
CreateMouseEvent(int type, int buttons, float x, float y, const std::string& modifiers_str) {
...
std::unique_ptr<blink::WebKeyboardEvent> TranslateKeyEvent(
	...
	std::unique_ptr<blink::WebKeyboardEvent> result = std::make_unique<blink::WebKeyboardEvent>();
  ...
  if (ui::DomCodeToUsLayoutDomKey(domCode, web_event_flags, &domKey, &dummy_code))
