...
#include "electron/third_party/libreofficekit/LibreOfficeKit.h"
#include "electron/third_party/libreofficekit/LibreOfficeKitEnums.h"
#include "electron/third_party/libreofficekit/LibreOfficeKitInit.h"
...
class Document {
 ...
 public:
  /// A lok::Document is typically created by the lok::Office::documentLoad()
  /// method.
  explicit Document(LibreOfficeKitDocument* pDoc) : mpDoc(pDoc) {}

  ~Document() { mpDoc->pClass->destroy(mpDoc); }
   ...
   */
  int getDocumentType() { return mpDoc->pClass->getDocumentType(mpDoc); }
   ...
   * or pages for Writer.
   */
  int getParts() { return mpDoc->pClass->getParts(mpDoc); }
  ...
  int getPart() { return mpDoc->pClass->getPart(mpDoc); }

  /// Set the current part of the document.
  void setPart(int nPart) { mpDoc->pClass->setPart(mpDoc, nPart); }
  ...
  void setPartMode(int nMode) { mpDoc->pClass->setPartMode(mpDoc, nMode); }
   ...
   *
   * @param pBuffer pointer to the buffer, its size is determined by
   * nCanvasWidth and nCanvasHeight.
   * @param nCanvasWidth number of pixels in a row of pBuffer.
   * @param nCanvasHeight number of pixels in a column of pBuffer.
   * @param nTilePosX logical X position of the top left corner of the rendered
   * rectangle, in TWIPs.
   * @param nTilePosY logical Y position of the top left corner of the rendered
   * rectangle, in TWIPs.
   ...
   * @param nWindowid
   */
  void postWindow(unsigned nWindowId,
                  int nAction,
   ...
   */
  int getTileMode() { return mpDoc->pClass->getTileMode(mpDoc); }
   ...
   */
  void initializeForRendering(const char* pArguments = nullptr) {
   ...
   * @param nCharCode contains the Unicode character generated by this event or
   * 0
   * @param nKeyCode contains the integer code representing the key of the event
   ...
   * (non-zero for control keys)
   */
  void postWindowKeyEvent(unsigned nWindowId,
                          int nType,
                          int nCharCode,
                          int nKeyCode) {
    mpDoc->pClass->postWindowKeyEvent(mpDoc, nWindowId, nType, nCharCode,
                                      nKeyCode);
  }
   ...
   * @param nModifier: which keyboard modifier: (see include/vcl/vclenum.hxx for
   * possible values)
   */
  void postMouseEvent(int nType,
                      int nX,
                      int nY,
                      int nCount,
                      int nButtons,
   ...
   * possible values)
   */
  void postWindowMouseEvent(unsigned nWindowId,
                            int nType,
                            int nX,
                            int nY,
                            int nCount,
                            int nButtons,
                            int nModifier) {
    mpDoc->pClass->postWindowMouseEvent(mpDoc, nWindowId, nType, nX, nY, nCount,
                                        nButtons, nModifier);
  }
   ...
   */
  void sendDialogEvent(unsigned long long int nWindowId,
   ...
   */
  void postUnoCommand(const char* pCommand,
                      const char* pArguments = nullptr,
   ...
   * @param pUsedMimeType output parameter to inform about the determined format
   * (suggested one or plain text).
   */
  char* getTextSelection(const char* pMimeType,
   ...
   */
  int getSelectionType() { return mpDoc->pClass->getSelectionType(mpDoc); }

  /**
   * Gets the type of the selected content and possibly its text.
   *
   * This function is a more efficient combination of getSelectionType() and
   * getTextSelection(). It returns the same as getSelectionType(), and
   * additionally if the return value is LOK_SELTYPE_TEXT then it also returns
   * the same as getTextSelection(), otherwise pText and pUsedMimeType are
   * unchanged.
   *
   * @param pMimeType suggests the return format, for example
   * text/plain;charset=utf-8.
   ...
   */
  int getSelectionTypeAndText(const char* pMimeType,
                              char** pText,
                              char** pUsedMimeType = nullptr) {
    if (LIBREOFFICEKIT_DOCUMENT_HAS(mpDoc, getSelectionTypeAndText))
      return mpDoc->pClass->getSelectionTypeAndText(mpDoc, pMimeType, pText,
                                                    pUsedMimeType);
   ...
   * binary streams.
   *
   * NB. returns a complete set of possible selection types if nullptr is passed
   * for pMimeTypes.
   *
   * @param pMimeTypes passes in a nullptr terminated list of mime types to
   * fetch
  ...
  bool getClipboard(const char** pMimeTypes,
                    ...
                    size_t** pOutSizes,
                    char*** pOutStreams) {
    return mpDoc->pClass->getClipboard(mpDoc, pMimeTypes, pOutCount,
                                       pOutMimeTypes, pOutSizes, pOutStreams);
  }
  ...
  bool setClipboard(const size_t nInCount,
    ...
    return mpDoc->pClass->setClipboard(mpDoc, nInCount, pInMimeTypes, pInSizes,
                                       pInStreams);
  }
   ...
   */
  void resetSelection() { mpDoc->pClass->resetSelection(mpDoc); }
   ...
   */
  void setClientZoom(int nTilePixelWidth,
   ...
   */
  void destroyView(int nId) { mpDoc->pClass->destroyView(mpDoc, nId); }

  /**
   * Set an existing view of an existing document as current.
   * @param nId a view ID, returned by createView().
   */
  void setView(int nId) { mpDoc->pClass->setView(mpDoc, nId); }

  /**
   * Get the current view.
   * @return a view ID, previously returned by createView().
   */
  int getView() { return mpDoc->pClass->getView(mpDoc); }

  /**
   * Get number of views of this document.
   */
  int getViewsCount() { return mpDoc->pClass->getViewsCount(mpDoc); }
  ...
  unsigned char* renderFont(const char* pFontName,
    ...
    if (LIBREOFFICEKIT_DOCUMENT_HAS(mpDoc, renderFontOrientation))
      return mpDoc->pClass->renderFontOrientation(
          mpDoc, pFontName, pChar, pFontWidth, pFontHeight, pOrientation);
    else
  ...
  void paintPartTile(unsigned char* pBuffer,
                     ...
                     const int nTileWidth,
                     const int nTileHeight) {
    return mpDoc->pClass->paintPartTile(mpDoc, pBuffer, nPart, nCanvasWidth,
                                        nCanvasHeight, nTilePosX, nTilePosY,
   ...
   * otherwise.
   */
  bool getViewIds(int* pArray, size_t nSize) {
   ...
   */
  void postWindowExtTextInputEvent(unsigned nWindowId,
                                   int nType,
                                   const char* pText) {
  ...
  void paintTileToCGContext(void* rCGContext,
                            ...
                            const int nTileWidth,
                            const int nTileHeight) {
    return mpDoc->pClass->paintTileToCGContext(
        mpDoc, rCGContext, nCanvasWidth, nCanvasHeight, nTilePosX, nTilePosY,
        nTileWidth, nTileHeight);
  }
#endif  // IOS
  ...
  bool insertCertificate(const unsigned char* pCertificateBinary,
    ...
    return mpDoc->pClass->insertCertificate(
        mpDoc, pCertificateBinary, pCertificateBinarySize, pPrivateKeyBinary,
   ...
   */
  int getSignatureState() { return mpDoc->pClass->getSignatureState(mpDoc); }
   ...
   * @param nX horizontal position in document coordinates
   * @param nY vertical position in document coordinates
   * @param nOffset difference value from when the gesture started to current
   * value
   */
  void postWindowGestureEvent(unsigned nWindowId,
                              ...
                              int nY,
                              int nOffset) {
    return mpDoc->pClass->postWindowGestureEvent(mpDoc, nWindowId, pType, nX,
                                                 nY, nOffset);
  }
   ...
   */
  void resizeWindow(unsigned nWindowId, const int width, const int height) {
   ...
   */
  bool renderSearchResult(const char* pSearchResult,
                          unsigned char** pBitmapBuffer,
                          int* pWidth,
                          int* pHeight,
                          size_t* pByteSize) {
    return mpDoc->pClass->renderSearchResult(
        mpDoc, pSearchResult, pBitmapBuffer, pWidth, pHeight, pByteSize);
  }
...
class Office {
 ...
 public:
  ...
  explicit Office(LibreOfficeKit* pThis) : mpThis(pThis) {}

  ~Office() { mpThis->pClass->destroy(mpThis); }
   ...
   */
  Document* documentLoad(const char* pUrl,
                         const char* pFilterOptions = nullptr) {
    LibreOfficeKitDocument* pDoc = nullptr;

    if (LIBREOFFICEKIT_HAS(mpThis, documentLoadWithOptions))
      pDoc =
          mpThis->pClass->documentLoadWithOptions(mpThis, pUrl, pFilterOptions);
    else
  ...
  /// Returns the last error as a string, the returned pointer has to be freed
  /// by the caller.
  char* getError() { return mpThis->pClass->getError(mpThis); }
   ...
   */
  void freeError(char* pFree) { mpThis->pClass->freeError(pFree); }
   ...
   * @since LibreOffice 6.0
   */
  char* getFilterTypes() { return mpThis->pClass->getFilterTypes(mpThis); }
   ...
   */
  char* getVersionInfo() { return mpThis->pClass->getVersionInfo(mpThis); }
  ...
  bool signDocument(const char* pURL,
                    ...
                    const unsigned char* pPrivateKeyBinary,
                    const int nPrivateKeyBinarySize) {
    return mpThis->pClass->signDocument(
        mpThis, pURL, pCertificateBinary, nCertificateBinarySize,
...
inline Office* lok_cpp_init(const char* pInstallPath,
                            const char* pUserProfileUrl = nullptr) {
