...
typedef enum { LOK_PARTMODE_SLIDES, LOK_PARTMODE_NOTES } LibreOfficeKitPartMode;

typedef enum { LOK_TILEMODE_RGBA, LOK_TILEMODE_BGRA } LibreOfficeKitTileMode;

typedef enum { LOK_WINDOW_CLOSE, LOK_WINDOW_PASTE } LibreOfficeKitWindowAction;
...
typedef enum {
   ...
   * height", and a 'misspelledWord' element represented by an integer value:
   * '1' when a misspelled word is at the cursor position, '0' when the word is
   * not misspelled.
   */
   ...
   *     "success": true/false,   // when the result is "don't know", this is
   * missing
   *     // TODO "result": "..."  // UNO Any converted to JSON (not implemented
   * yet)
   * }
   */
   ...
   *     "kind": "network" etc.
   *     "code": a structured 32-bit error code, the ErrCode from LibreOffice's
   * <tools/errcode.hxx> "message": freeform description
   * }
   */
   ...
   * ".uno:Something1", "enabled": "true" }, { "text": "label text2", "type":
   * "command", "command": ".uno:Something2", "enabled": "false" }, { "type":
   * "separator" }, { "text": "label text2", "type": "menu", "menu": [ { ... },
   * { ... }, ... ] },
   *         ...
   *     ]
   * }
   ...
   *
   *     {"text": "label text3", "type": "command", "command":
   * ".uno:Something3", "checktype": "checkmark|radio|auto", "checked":
   * "true|false"}
   */
   ...
   * selected text. Payload format is JSON. Example: { "mimeType": "text/plain",
   ...
   * {
   *     "marks": [
   *         { "rectangle": "3825, 3315, 1245, 2010", "color": "0000ff", "part":
   * "0" }, { "rectangle": "8925, 4335, 2520, 735", "color": "ff0000", "part":
   * "0" },
   *         ...
   *     ]
   * }
   ...
   * displayed and also contains all information that the popup window needs.
   *
   * The payload example:
   * {
   *      "action": "show",
   ...
   *
   * The payload format is JSON: { "title": "title text", "content": "content
   * text" }
   */
